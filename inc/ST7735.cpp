...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
    ...
    for(y=w - 1; y>=0; y--){
        for(x=0; x<h; x++){
            uint16_t color;
            if (ANIRUDDH_SCREEN) color = ST7735_SwapColor(image[x*h + y]);
            else color = ~(image[x*w + y]);
                                            // send the top 8 bits
            SPI_OutData((uint8_t)(color >> 8));
...
void ST7735_OutChar(char ch, uint8_t size, uint16_t color){
  if((ch == 10) || (ch == 13) || (ch == 27)){ // New line
    ...
    if (!ANIRUDDH_SCREEN) StTextColor = ~(ST7735_SwapColor(StTextColor));
    ST7735_DrawString(0,StY,(char *)"                     ",StTextColor);
    return;
  }

  uint16_t bg_color = ST7735_BLACK;
  if (!ANIRUDDH_SCREEN) bg_color = ~ST7735_BLACK;

  if (!ANIRUDDH_SCREEN) color = ~(ST7735_SwapColor(color));

  ST7735_DrawCharS(StX*10,StY*6,ch,color,bg_color, size);
  if(StY>27 - size){
    StY = 27 - size;
    ST7735_DrawCharS(StX*10,StY*6,'*',ST7735_RED,bg_color, size);
  }
  StY += size;
  return;
}
